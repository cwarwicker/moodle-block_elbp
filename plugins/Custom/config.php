...
$PAGE->set_url($CFG->wwwroot . '/' . $OBJ->getConfigPath());
$PAGE->set_title( $OBJ->getTitle() . ' ' . get_string('config', 'block_elbp') );
...
$PAGE->navbar->add( $OBJ->getTitle(), $CFG->wwwroot . '/' . $OBJ->getConfigPath(), navigation_node::TYPE_CUSTOM);
...
if ($view == 'permissions'){
    $roles = $DB->get_records("role", array(), "id ASC");
    $TPL->set("roles", $roles);
} 
