...
[[Textbox: |name = "Student Comments"| |label = "Student Comments"|| |validation = {type = "NOT_EMPTY"}| |display = "main"]]';
    ...
    public function getAllReportingData($students, $elements)
    {
        ...
        $year = date('Y');
        if ($month >= 9){
            $startOfTerm = strtotime("01-09-{$year} 00:00:00");
        } else {
            $year--;
            $startOfTerm = strtotime("01-09-{$year} 00:00:00");
        }
        ...
        $totalTutorials = 0;
        $totalWithTutorials = 0;
        $totalWithTutorialsSinceStartOfTerm = 0;
        $latestTutorial = 0;
        ...
        foreach($students as $student)
        {
            ...
            if ($tutorials)
            {
                ...
                $totalWithTutorials++;
                $studHasSomeThisYear = false;
                
                foreach($tutorials as $tutorial)
                {
                    
                    $unix = $tutorial->getSetTime();
                    
                    if ($unix > $startOfTerm && !$studHasSomeThisYear)
                    {
                        $totalWithTutorialsSinceStartOfTerm++;
                        $studHasSomeThisYear = true;
                    }
                    
                    if ($unix > $latestTutorial)
                    {
                        $latestTutorial = $unix;
                    }
        ...
        // Totals
        $data['reports:tutorials:numtutorials'] = $totalTutorials;
        
        
        // Averages
        $data['reports:tutorials:avgtutorials'] = ($totalTutorials) ? round( ($totalTutorials / $totalStudents), 2 ) : 0;
        
        
        // Percentages
        $data['reports:tutorials:percentwithtutorials'] = ($totalTutorials) ? round( ($totalWithTutorials / $totalStudents) * 100, 2 ) : 0;
        $data['reports:tutorials:percentwithtutorialssincestartofterm'] = ($totalWithTutorialsSinceStartOfTerm) ? round( ($totalWithTutorialsSinceStartOfTerm / $totalStudents) * 100, 2 ) : 0;
        
        // Other
        $data['reports:tutorials:lasttutorial'] = ($latestTutorial > 0) ? date('d-m-Y', $latestTutorial) : '-';
