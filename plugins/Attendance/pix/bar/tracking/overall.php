...
$unix = $attendanceObject->getTrackingFromDate();
$trackingDays = $attendanceObject->getSetting('track_days');
$dates = array();
$vals = array();
...
while ($unix <= $end)
{
    ...
    foreach($__types as $type)
    {
        ...
        $period = "Total";# testing
        $value = $attendanceObject->getRecordHistory( array("type"=>$type, "courseid"=>null, "period"=>$period), $unix );
        $vals[$type][] = $value;
        
    }
    
    $unix = $unix + ($trackingDays * 86400);
...
if (!in_array($today, $dates))
{
    ...
    foreach($__types as $type)
    {
        ...
        $period = "Total";# testing
        $value = $attendanceObject->getRecord( array("type"=>$type, "courseid"=>null, "period"=>$period) );
        $vals[$type][] = $value;
...
foreach($__types as $type)
{
    $myData->addPoints($vals[$type],"Serie{$num}");
