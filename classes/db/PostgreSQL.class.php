 ...
 *
 * This class is for querying postgres databases.
 *
 * @package     block_elbp
...
// TODO: This is kind of pointless, as I could just use the core moodle database API which supports PostgreSQL.

/**
 *
 */
class PostgreSQL extends Manager {
     ...
     * @param type $db
     */
    private function connect_pdo_pgsql($host, $user, $pass = "", $db = "") {
        ...
        try {
            ...
            $DBH = new \PDO("pgsql:{$cStr}", $user, $pass);
     ...
     */
    private function disconnect_pdo_pgsql() {
        $this->disconnect_pdo();
    }
     ...
     * @return type
     */
    private function query_pdo_pgsql($sql, $params) {
     ...
     * @return type
     */
    private function execute_pdo_pgsql($sql, $params) {
    ...
    public function select($table, $where = null, $fields = "*", $order = null, $limit = null) {
        ...
        if (is_array($where)) {
            ...
            foreach ($where as $name => $value) {
                $sql .= " ".$this->wrapValue($name)." ILIKE ? AND ";
                $params[] = $value;
            }
        }
     ...
     * @return type
     */
    private function getRecordSet_pdo_pgsql($query) {
    ...
    public function update($table, $data, $where = null, $limit = null) {
        ...
        if (!is_null($where)) {
            ...
            foreach ($where as $field => $value) {
                $sql .= ' '.$this->wrapValue($field).' ILIKE ? AND';
                $params[] = $value;
            }
        ...
        if (!is_null($limit)) {
            if (is_null($where)) {
                $sql .= ' WHERE ctid IN ( SELECT ctid FROM '.$this->wrapValue($table).' LIMIT '.$limit.' ) ';
            } else {
                $sql .= ' AND ctid IN ( SELECT ctid FROM '.$this->wrapValue($table).' LIMIT '.$limit.' ) ';
            }
        }
     ...
     * @return type
     */
    private function fetch_pdo_pgsql($qry) {
