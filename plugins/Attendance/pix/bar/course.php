...
if (!isset($_GET['studentID']) || !isset($_GET['courseID'])) exit;
...
$course = $ELBP->getDB()->getCourse( array("type"=>"id", "val"=>$courseID) );
if (!$course) exit;
...
foreach ($__types as $type)
{
    ...
    {
        $val = $attendanceObject->getRecord( array("period"=>$period, "type"=>$type, "courseid"=>$course->id) );
...
, "R"=>0, "G"=>0, "B"=>0);
$myPicture->drawText(350,25,$course->shortname,$TextSettings);
